...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// 处理f值为null时
		if(f == null){
			return findLeafPage(tid, dirtypages, internalPage.getChildId(0), perm, f);
		}
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// 将原page后半部分的tuple给newPage
		int newPageTupleNum = page.getNumTuples() / 2;
		Iterator<Tuple> it = page.reverseIterator();
		for(int i=0; i < newPageTupleNum && it.hasNext(); i++){
			Tuple t = it.next();
			page.deleteTuple(t);
		...
		Field midTupleField = midTuple.getField(keyField);
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midTupleField);
		BTreeEntry newEntry = new BTreeEntry(midTupleField, page.getId(), newPage.getId());
		parentPage.insertEntry(newEntry);

		// 更新叶节点的左右指针, 类似双向数组的插入
		newPage.setLeftSiblingId(page.getId());
		newPage.setRightSiblingId(page.getRightSiblingId());

		if(page.getRightSiblingId() != null){
			BTreeLeafPage nextLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			nextLeafPage.setLeftSiblingId(newPage.getId());
		}
		...
		// 判断插入在哪个页
		if(field.compare(Op.LESS_THAN, midTupleField)){
			return page;
		}else{
			return newPage;
		}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// 找一半
		int newPageEnrtyNum = page.getNumEntries() / 2;
		Iterator<BTreeEntry> it = page.reverseIterator();

		for(int i=0; i<newPageEnrtyNum && it.hasNext(); i++){
			BTreeEntry entry = it.next();
		...
		// 设置中间的entry的孩子 左右分别是原page和新的page
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int targetTupleCount = (pageTupleNum + siblingTupleNum + 1) / 2; // 约等于向上取整
		int stealTupleNum = targetTupleCount - pageTupleNum;
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int targetCount = (pageEntryNum + siblingEntryNum + 1) / 2 ; // 约等于向上取整
		int stealEntryNum = targetCount-pageEntryNum;

		Iterator<BTreeEntry> siblingIt = leftSibling.reverseIterator();
		for(int i=0; i<stealEntryNum; i++){

			BTreeEntry entry = siblingIt.next();
			leftSibling.deleteKeyAndRightChild(entry);

			BTreeEntry pullParentEntry = new BTreeEntry(parentEntry.getKey(), entry.getRightChild(), page.getChildId(0));

			page.insertEntry(pullParentEntry);

			updateParentPointer(tid, dirtypages, page.getId(), pullParentEntry.getLeftChild());
			updateParentPointer(tid, dirtypages, page.getId(), pullParentEntry.getRightChild());
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry> siblingIt = rightSibling.iterator();
		for(int i=0; i<stealEntryNum; i++){

			BTreeEntry entry = siblingIt.next();
			rightSibling.deleteKeyAndLeftChild(entry);

			BTreeEntry pullParentEntry = new BTreeEntry(parentEntry.getKey(), page.getChildId(page.getNumEntries()), entry.getLeftChild());
